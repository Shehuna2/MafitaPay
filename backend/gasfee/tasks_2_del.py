...
def process_crypto_order_sync(order_id):
    """
    Synchronous version of the previous Celery task.
    Call this from your view or management command when you want to process an order.
    """
    try:
        ...
        if order.status == "pending":
            ...
            try:
                send_mail(
                    ...
                    "no-reply@mafitapay.com",
                    [order.user.email],
                    fail_silently=True,
                )
            except Exception as exc:
                logger.exception("Failed to send completion email for order %s: %s", order_id, exc)
