...
export default function Dashboard() {
  ...
  const [copiedId, setCopiedId] = useState(null); // ← NEW: for copy feedback
  ...
  // FETCH ONLY WALLET (includes virtual_accounts)
  ...
  useEffect(() => {
    fetchWallet();
    const interval = setInterval(fetchWallet, 30000); // Auto-refresh every 30s
    ...
    return (
      <>
        <style>{`
          ...
          .shimmer { background: linear-gradient(90deg, #1a1a2e 25%, #2a2a3e 50%, #1a1a2e 75%); background-size: 200% 100%; animation: shimmer 1.8s infinite linear; }
        `}</style>
        <div className="min-h-screen bg-gray-900 text-white px-3 py-6">
          <div className="w-full max-w-md mx-auto mb-6">
            <div className="bg-indigo-600/30 backdrop-blur-2xl p-6 rounded-3xl border border-indigo-600/20">
              <div className="h-10 w-3/4 bg-gray-700/50 rounded shimmer mx-auto mb-6" />
  ...
  return (
    <>
      <style>{`
        ...
        .shadow-wallet { box-shadow: 0 8px 32px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.1); }
        .haptic-feedback { transition: transform 0.1s; }
          ...
          <div className="relative bg-indigo-600/30 backdrop-blur-2xl p-5 sm:p-8 rounded-3xl shadow-wallet border border-indigo-600/20 transform transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-1">
                ...
                <button onClick={() => setShowBalance(v => !v)} className="p-2 rounded-xl bg-white/10 hover:bg-white/20 backdrop-blur-sm transition haptic-feedback">
